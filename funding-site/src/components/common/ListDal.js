    ...
    if(res){
        ...
        return (
            <div className="best_text">
                <Link to={`/projectDetail/${pac.p_addr}`}><h2>{pac.p_title}</h2></Link>
                <p>{pac.p_cate}
                {pac.p_type === "C" && (<img src={classIcon} className="classOrprojectIcon2" alt="이미지"/>)}
                {pac.p_type === "P" && (<img src={projectIcon} className="classOrprojectIcon2" alt="이미지"/>)}
                </p>
                <h3>{pac.p_explain}</h3>
                {pac.p_type === "C" && (
                    <div className="per_line" style={{width: `100%`}}></div>
                )}
                {pac.p_type === "P" && (
                    <span>
