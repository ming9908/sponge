...
const Mpmyproject = ({user, pac, history, res}) => {
    ...
    if( pac ){
        console.log(pac)
        console.log(user)
        madebymeproject = pac.filter(t => t.p_type === "P" && t.p_maker.u_id === user.u_id);
        console.log(madebymeproject)
    }
    return(
        <>
            <div className="myPageTitle">내가 제작한 프로젝트</div>
            <div className="myPageExpain">내가 제작한 프로젝트 내역입니다.</div>
                ...
                {madebymeproject.map(p => (
                    ...
                    {p.p_state === "Y" && (
                        <span>
                            ...
                            <Link to={`/pacDashboard/${p.p_addr}`}><div style={{'textAlign' : 'center'}}><button className="deagib"><span>대시보드로 가기</span></button></div></Link>
                        </span>
                    )}
...
export default withRouter(Mpmyproject);
